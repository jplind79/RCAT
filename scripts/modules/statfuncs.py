...
def prob_of_exceed(data, keepdims=False, thr=None, bootstrap=False, nmc=500,
                   block=1, ci_level=95, nproc=1):
    """
    ...
    ci: dict
        data dictionary with confidence levels
    """
    import pandas as pd
            ...
            if bootstrap:
                if isinstance(pdata, (np.ma.MaskedArray, np.ndarray)):
                    if pdata.ndim > 2 and not keepdims:
                        ...
                        bs_odata = []
                        for bs in range(nmc):
                            print("Generating POE for bootsample\n{}".format(
                                bs))
                            pool = mp.Pool(processes=nproc)
                            comps = pool.starmap_async(_mproc_get_bootsamples,
                                                       iterlist)
                            ...
                            indata.sort()
                            ser_cdf = pd.Series(cum_dist, index=indata)
                            bs_odata.append(1. - ser_cdf)
                        bs_odata = np.array(bs_odata)
                    else:
                        btsamples = block_bootstr(indata, block=block,
                                                  nrep=nmc, nproc=nproc)
                        bs_odata = []
                ...
                alpha = 100 - ci_level
                min_level = [np.nanpercentile(bs_odata[:, x], alpha/2.)
                             for x in range(bs_odata.shape[1])]
                max_level = [np.nanpercentile(bs_odata[:, x], 100-alpha/2.)
                             for x in range(bs_odata.shape[1])]
            else:
                min_level = None
                max_level = None
    ...
    if keepdims:
        eop_o, ci_data = np.apply_along_axis(eop_calc, 0, data, thr)
        eop = reduce((lambda x, y: np.c_[x, y]),
                     eop_o.ravel()).reshape(eop_o[0, 0].size,
                                            eop_o.shape[0], eop_o.shape[1])
    else:
        eop, ci_data = eop_calc(data, thr=thr)
